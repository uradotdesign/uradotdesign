...
const ContactSchema = z.object({
  ...
  contact_preference: z.enum(['email','phone','signal']).optional(),
  contact_preferences: z.array(z.enum(['email','phone','signal'])).min(1).max(3).optional(),
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const submissionData = {
      ...
      contact_preference: data.contact_preferences?.[0] || data.contact_preference || "email",
      contact_preferences: [...new Set(data.contact_preferences || [data.contact_preference || 'email'])],
